...
class PeersBloc extends Bloc<dynamic, PeersState> {
  ...
  Stream<PeersState> _mapConsumerAddToState(PeerAddConsumer event) async* {
    ...
    if (kIsWeb) {
      ...
      if (event.consumer.kind == 'video') {
        ...
        newPeers[event.peerId]!.renderer!.srcObject = newPeers[event.peerId]!.video!.stream;
      }
      ...
      if (event.consumer.kind == 'audio') {
        ...
        if (newPeers[event.peerId]!.video == null) {
          newPeers[event.peerId]!.renderer!.srcObject = newPeers[event.peerId]!.audio!.stream;
        }
      }
    } else {
      if (event.consumer.kind == 'video') {
  ...
  Stream<PeersState> _mapConsumerRemoveToState(PeerRemoveConsumer event) async* {
    final Map<String, Peer> newPeers = Map<String, Peer>.of(state.peers);
    final Peer? peer = newPeers.values.firstWhereOrNull((p) => p.consumers.contains(event.consumerId));
    ...
    if (peer != null) {
      if (kIsWeb) {
        if (peer.audio?.id == event.consumerId) {
          ...
          if (peer.audio != null) {
            newPeers[peer.id]!.renderer!.srcObject = newPeers[peer.id]!.audio!.stream;

...
class RoomClientRepository {
  ...
  void enableWebcam() async {
    ...
    if (_mediasoupDevice == null ||
        _mediasoupDevice!.loaded == false ||
        _mediasoupDevice!.canProduce(RTCRtpMediaType.RTCRtpMediaTypeVideo) == false) {
      return;
    }
  ...
  void enableMic() async {
    if (_mediasoupDevice == null ||
        _mediasoupDevice!.loaded == false ||
        _mediasoupDevice!.canProduce(RTCRtpMediaType.RTCRtpMediaTypeAudio) == false) {
      return;
    }
  ...
  void _connectWebRtcTransport(data) {
    print("asda");
    _webSocket!.socket
        .request(
  ...
  Future<void> _joinRoom() async {
    try {
      ...
      if (_consume) {
        ...
        _recvTransport!.on('connect', _connectWebRtcTransport);
